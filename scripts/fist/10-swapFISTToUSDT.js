...
import {
  previewFistToUsdtSwap,
  swapFistToUsdt,
  formatUsdtPerFist,
  estimateUsdtOut,
} from "../../lib/fist/pancakeV2.js";
...
function printHelp() {
  ...
  --min-fist MIN          Skip below (default: 1, BSC 6 decimals)
...
async function main() {
  ...
  const minFistWei = parseUnits(String(args.minFist), FIST_BSC.FIST_DECIMALS);
  ...
  for (const job of jobs) {
    const preview = await previewFistToUsdtSwap(provider, job, {
  ...
  const oneFist = parseUnits("1", FIST_BSC.FIST_DECIMALS);
  const marketUsdtPerFist = await estimateUsdtOut(provider, oneFist);
  const marketRate = formatUsdtPerFist(oneFist, marketUsdtPerFist);

  console.log("\nMarket quote (PancakeSwap V2):");
  console.log(`  1 FIST ≈ ${marketRate} USDT`);
  ...
  for (const p of previews) {
    if (p.status === "ready") {
      console.log(
        `  ${p.label} (${p.address})\n` +
          `    ${p.fist_in} FIST → ~${p.usdt_estimated} USDT ` +
  ...
  const totalFist = ready.reduce((s, p) => s + p.fist_in_wei, 0n);
  const totalUsdt = ready.reduce((s, p) => s + p.usdt_estimated_wei, 0n);
  console.log(
    `\nTotal: ${ready.length} wallet(s), ` +
      `${fmtAmount(totalFist, FIST_BSC.FIST_DECIMALS)} FIST → ~${fmtAmount(totalUsdt)} USDT (estimated)`
  );

  if (!args.yes && !args.dryRun) {
    const ok = await confirmProceed(
      "\nProceed with FIST -> USDT swaps at the quotes above? [y/N] "
    );
    if (!ok) {
      console.log("aborted");
      process.exit(0);
  ...
  const logger = createLogger("fistSwapFISTToUSDT", { projectRoot: PROJECT_ROOT });
  const results = [];
  ...
  for (let idx = 0; idx < jobs.length; idx += 1) {
    ...
    let result;
    try {
      result = await swapFistToUsdt(provider, job, {
        amountArg: args.amount,
        minFistWei,
        slippageBps: args.slippageBps,
