...
import {
  previewBtcdToFistSwap,
  swapBtcdToFist,
  formatFistPerBtcd,
  estimateFistOut,
} from "../../lib/fist/pgpRouter.js";
...
async function main() {
  ...
  for (const job of jobs) {
    const preview = await previewBtcdToFistSwap(provider, job, {
  ...
  const marketFistPerBtcd = await estimateFistOut(provider, oneBtcd);
  const marketRate = formatFistPerBtcd(oneBtcd, marketFistPerBtcd);

  console.log("\nMarket quote (PGARouterV2):");
  console.log(`  1 BTCD ≈ ${marketRate} FIST`);
  ...
  for (const p of previews) {
    if (p.status === "ready") {
      console.log(
        `  ${p.label} (${p.address})\n` +
          `    ${p.btcd_in} BTCD → ~${p.fist_estimated} FIST ` +
          `(min ${p.fist_min_out} FIST @ ${slippagePct}% slippage)`
      );
    } else {
  ...
  const totalFist = ready.reduce((s, p) => s + p.fist_estimated_wei, 0n);
  console.log(
    `\nTotal: ${ready.length} wallet(s), ` +
      `${fmtAmount(totalBtcd)} BTCD → ~${fmtAmount(totalFist)} FIST (estimated)`
  );

  if (!args.yes && !args.dryRun) {
    const ok = await confirmProceed(
      "\nProceed with BTCD -> FIST swaps at the quotes above? [y/N] "
    );
    if (!ok) {
      console.log("aborted");
      process.exit(0);
  ...
  const logger = createLogger("fistSwapBTCDToFIST", { projectRoot: PROJECT_ROOT });
  const results = [];
  ...
  for (let idx = 0; idx < jobs.length; idx += 1) {
    ...
    let result;
    try {
      result = await swapBtcdToFist(provider, job, {
        amountArg: args.amount,
        minBtcdWei,
        slippageBps: args.slippageBps,
